...
function AiUpload() {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', backgroundColor: '#F3F4F4' }}>
      ...
      {/* 상단 텍스트 가이드 */}
      <p style={{ position: 'absolute', textAlign: 'center', top: 150, left: '28px', color: '#002738', fontSize: '16px' }}>
        {name}님의 체형 사진과 체형 고민을 추가해 주세요
      </p>

      <p style={{ position: 'absolute', textAlign: 'center', top: 180, left: '53px', color: '#002738', fontSize: '16px' }}>
        체형이 잘 드러나는 전신 사진을 업로드 하면<br />정확도가 올라가요!
      </p>

      {/* 에러 메시지 */}
      {error && (
        <p style={{ position: 'absolute', top: '780px', left: '33px', color: '#e53e3e', fontSize: '14px', margin: 0 }}>
          {error}
        </p>
      )}
