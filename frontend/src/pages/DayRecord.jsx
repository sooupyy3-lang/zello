...
function DayRecord() {
  ...
  return (
    <div style={{ width: '100%', minHeight: '100dvh', position: 'relative', fontFamily: 'inherit' }}>

      {/* 배경 이미지 */}
      <img
        src={RecordImage} alt="background"
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%' }}>
        ...
        <div style={{
          ...
          padding: '0 20px',
          paddingTop: 'clamp(100px, 25vw, 139px)',
          position: 'relative',
        }}>
          ...
          <p style={{ margin: 0, fontSize: 'clamp(18px, 6vw, 24px)', fontWeight: '800', color: '#002738' }}>
        ...
        <div style={{
          ...
          <img
            ...
            style={{
              width: 'clamp(180px, 62vw, 251px)',
              marginTop: 'clamp(32px, 8vw, 50px)',
              marginBottom: 'clamp(20px, 5vw, 29px)',
            }}
          />
          ...
          {loading ? (
            ...
            <p style={{ fontSize: 'clamp(15px, 4.5vw, 18px)', fontWeight: '700', color: '#002738', textAlign: 'center' }}>
              이 날은 운동 기록이 없어요 😴
            </p>
          ) : (
            <>
              <div style={{ width: '100%', textAlign: 'center', marginBottom: '16px' }}>
                <p style={{ margin: '0 0 6px', fontSize: 'clamp(16px, 5vw, 20px)', fontWeight: '700', color: '#002738' }}>
                  총 운동 시간 - {totalHour > 0 ? `${totalHour}시간 ` : ''}{totalMin > 0 ? `${totalMin}분 ` : ''}{totalRemSec > 0 || totalSec === 0 ? `${totalRemSec}초` : ''}
                </p>
                {session.tracks?.map((t, i) => (
                  <p key={i} style={{ margin: '2px 0', fontSize: 'clamp(15px, 4.7vw, 19px)', fontWeight: '700', color: '#002738' }}>
              ...
              <div style={{ width: '100%', textAlign: 'center' }}>
                ...
                <p style={{ margin: '12px 0 0', fontSize: 'clamp(16px, 5vw, 20px)', fontWeight: '700', color: '#002738' }}>
                  총 {totalCal} kcal 소모
                </p>
              </div>
            </>
